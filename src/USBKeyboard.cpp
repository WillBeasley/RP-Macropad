...
USBKeyboard2::USBKeyboard2(bool connect, uint16_t vendor_id, uint16_t product_id, uint16_t product_release):
    USBHID(get_usb_phy(), 0, 0, vendor_id, product_id, product_release)
{
    _lock_status = 0;
}

USBKeyboard2::USBKeyboard2(USBPhy *phy, uint16_t vendor_id, uint16_t product_id, uint16_t product_release):
...
USBKeyboard2::~USBKeyboard2()
{
}

const uint8_t *USBKeyboard2::report_desc()
{
...
uint8_t USBKeyboard2::lock_status()
{
    return _lock_status;
}

int USBKeyboard2::_putc(int c)
{
    return key_code(c, keymap[c].modifier);
}

bool USBKeyboard2::key_code(uint8_t key, uint8_t modifier)
{
    _mutex.lock();
...
bool USBKeyboard2::media_control(MEDIA_KEY key)
{
    _mutex.lock();
...
const uint8_t *USBKeyboard2::configuration_desc(uint8_t index)
{
    if (index != 0) {
        return NULL;
    }
